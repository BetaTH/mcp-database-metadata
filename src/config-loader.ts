...
function readConfigFile(filePath: string): Partial<Config> | undefined {
	...
	try {
		...
		console.warn(
			`Aviso: Erro ao ler ou parsear o arquivo de configuração: ${filePath}`,
		);
		return undefined;
	}
}
...
export function loadConfiguration(cliPath?: string): Config | undefined {
	...
	if (Object.keys(mergedConfig).length === 0) {
		console.warn(
			"Aviso: Nenhuma configuração encontrada ou todas as configurações são inválidas. As ferramentas que dependem de configuração não serão registradas.",
		);
		return undefined;
	}
	...
	try {
		...
		console.warn(
			"Aviso: A configuração final mesclada é inválida. As ferramentas que dependem de configuração não serão registradas.",
		);
